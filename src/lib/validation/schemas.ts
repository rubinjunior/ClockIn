...
export const timeEntrySchema = z.object({
  ...
  const startsOn = formatInTimeZone(clockIn, "Asia/Jerusalem", "yyyy-MM-dd");
  const endsOn = formatInTimeZone(clockOut, "Asia/Jerusalem", "yyyy-MM-dd");
  if (startsOn > today || endsOn > today) context.addIssue({ code: "custom", path: ["clockOut"], message: "לא ניתן להזין דיווח ליום עתידי" });
});
