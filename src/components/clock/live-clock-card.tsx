...
export function LiveClockCard({ activeClockIn, workedMinutes = 0, expectedMinutes = 0 }: { activeClockIn?: string | null; workedMinutes?: number; expectedMinutes?: number }) {
  ...
  const toggle = () => startTransition(async () => { setFeedback(null); const result = clockIn ? await stopClock() : await startClock(); setFeedback({ ok: result.ok, message: result.message }); if (result.ok) setClockIn(clockIn ? null : String(result.entry?.clock_in ?? new Date().toISOString())); });
  return <section className="glass overflow-hidden rounded-[28px] p-5 sm:p-8" aria-labelledby="clock-title"><div className="flex items-start justify-between gap-4"><div><div className="mb-3 flex items-center gap-2 text-sm font-bold text-[var(--success)]"><span className={clockIn ? "status-dot is-active" : "size-2 rounded-full bg-[var(--surface-muted)]"}/>{clockIn ? he.clock.active : he.clock.inactive}</div><h2 id="clock-title" className="text-lg font-bold">{he.dashboard.title}</h2></div><span className="clock-orbit" aria-hidden><TimerReset className="text-[var(--primary)]" size={24}/></span></div>
    ...
    <button onClick={toggle} disabled={pending} className={`w-full ${clockIn ? "button-danger" : "button-primary"}`}>{pending ? <span className="spinner" aria-hidden /> : clockIn ? <CircleStop aria-hidden/> : <Play aria-hidden/>}{pending ? "מעדכן..." : clockIn ? he.clock.stop : he.clock.start}</button>{feedback && <p role={feedback.ok ? "status" : "alert"} className={`mt-3 rounded-2xl p-3 text-center text-sm font-bold ${feedback.ok ? "bg-[var(--success-soft)] text-[var(--success)]" : "bg-[var(--error-soft)] text-[var(--error)]"}`}>{feedback.message}</p>}
  </section>;
}
