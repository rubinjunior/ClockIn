...
import { ChevronLeft, ChevronRight, Clock, CircleCheck, CircleMinus, Gauge, Plane, Stethoscope } from "lucide-react";
...
function DayCard({ day }: { day: {date:string; expectedMinutes:number; workedMinutes:number; finalBalanceMinutes:number; sessions:number; future?:boolean} }) { return <article className="card p-4"><div className="flex items-center justify-between"><div><h3 className="font-extrabold">{new Intl.DateTimeFormat("he-IL",{weekday:"long",day:"numeric",month:"long"}).format(new Date(`${day.date}T12:00:00Z`))}</h3><p className="muted text-sm">{day.future ? "יום עתידי" : day.sessions ? `${day.sessions} דיווחים` : "אין דיווחים"}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${day.finalBalanceMinutes < 0 ? "bg-[var(--warning-soft)] text-[var(--warning)]":"bg-[var(--success-soft)] text-[var(--success)]"}`}>{day.future ? "לא נספר" : day.finalBalanceMinutes < 0 ? "חסרות שעות" : "הושלם"}</span></div><dl className="mt-4 grid grid-cols-3 gap-2 text-center"><div><dt className="muted text-xs">צפוי</dt><dd className="metric-value font-bold">{formatMinutes(day.expectedMinutes)}</dd></div><div><dt className="muted text-xs">בפועל</dt><dd className="metric-value font-bold">{formatMinutes(day.workedMinutes)}</dd></div><div><dt className="muted text-xs">מאזן</dt><dd className="metric-value font-bold">{formatMinutes(day.finalBalanceMinutes)}</dd></div></dl></article>; }
