...
test("אפשר ליצור שגרת עבודה חדשה ולקבל אישור", async ({ page }) => {
  ...
  await expect(page.getByText("שגרת העבודה נשמרה")).toBeVisible({ timeout: 10_000 });
});
...
test("טפסי ההגדרות שומרים ומציגים משוב", async ({ page }) => {
  ...
  await expect(page.getByText("הפרופיל נשמר")).toBeVisible({ timeout: 10_000 });
  ...
  await expect(page.getByText("הגדרת השכר נשמרה")).toBeVisible({ timeout: 10_000 });
  ...
  await expect(page.getByText("ההיעדרות נשמרה")).toBeVisible({ timeout: 10_000 });
  ...
  await expect(page.getByText("היום המיוחד נשמר")).toBeVisible({ timeout: 10_000 });

  await page.getByRole("button", { name: /תזכורות/ }).click();
  await page.getByRole("button", { name: "שמירת תזכורות" }).click();
  await expect(page.getByText("התזכורות נשמרו")).toBeVisible({ timeout: 10_000 });
});

test("מרכז ההגדרות מציג תחום אחד בכל פעם ללא גלילה אופקית", async ({ page }) => {
  await page.goto("/app/settings");
  await expect(page.getByRole("heading", { name: "שעות עבודה", level: 2 })).toBeVisible({ timeout: 10_000 });
  await expect(page.getByLabel("שם משתמש")).toHaveCount(0);
  expect(await page.locator("form").count()).toBeLessThan(5);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);

  await page.getByRole("button", { name: /פרופיל/ }).click();
  await expect(page.getByRole("heading", { name: "פרופיל", level: 2 })).toBeVisible({ timeout: 10_000 });
  await expect(page.getByLabel("שם משתמש")).toBeVisible({ timeout: 10_000 });
